...
def plot_attention_heatmap(data, x, y, head_id, ax):
    seaborn.heatmap(
        data, xticklabels=x, yticklabels=y, square=True, vmin=0.0, vmax=1.0, cbar=False, annot=True, fmt=".2f", ax=ax
    )
...
def visualize_attention_helper(attention_weights, source_sentence_tokens=None, target_sentence_tokens=None, title=""):
    ...
    assert (
        source_sentence_tokens is not None or target_sentence_tokens is not None
    ...
    for head_id, head_attention_weights in enumerate(attention_weights):
        ...
        plot_attention_heatmap(
            ...
            target_sentence_tokens if head_id % num_columns == 0 else [],
            head_id,

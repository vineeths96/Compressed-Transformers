...
def greedy_decoding(
    baseline_transformer, src_representations_batch, src_mask, trg_field_processor, max_target_tokens=100
):
    """
    ...
    trg_token_ids_batch = torch.tensor(
        [[trg_field_processor.vocab.stoi[tokens[0]]] for tokens in target_sentences_tokens], device=device
    )
    ...
    while True:
        ...
        predicted_log_distributions = baseline_transformer.decode(
            trg_token_ids_batch, src_representations_batch, trg_mask, src_mask
        )
        ...
        trg_token_ids_batch = torch.cat(
            (trg_token_ids_batch, torch.unsqueeze(torch.tensor(most_probable_last_token_indices, device=device), 1)), 1
        )
...
def get_beam_decoder(translation_config):
    ...
    def beam_decoding(
        ...
        src_representations_batch = src_representations_batch.repeat(1, beam_size, 1).view(
            beam_size * batch_size, -1, model_dimension
        )
        ...
        while True:
            ...
            # Shape = (B*BS, BS)
            latest_token_log_probs, most_probable_token_indices = torch.topk(
                predicted_log_distributions, beam_size, dim=-1, sorted=True
            )
            ...
            # Shape = (B, BS)
            new_hypothesis_log_probs, next_hypothesis_indices = torch.topk(
                hypotheses_pool_log_probs, beam_size, dim=-1, sorted=True
            )
            ...
            for b_idx, indices in enumerate(next_hypothesis_indices):
                for h_idx, token_index in indices:
                    ...
                    if had_eos[hypothesis_index]:
                        ...
                        new_trg_token_ids_batch[hypothesis_index, :-1] = trg_token_ids_batch[
                            b_idx * beam_size + row, :
                        ]
        ...
        for hypothesis_ids in trg_token_ids_batch_numpy:
            target_multiple_hypotheses_tokens.append(
                [trg_field_processor.vocab.itos[token_id] for token_id in hypothesis_ids]
            )
